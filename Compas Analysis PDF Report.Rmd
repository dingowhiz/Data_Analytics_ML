...
```
```{r}
#Hazard ratio based on race
control <- exp(-1.52554) / (1 + exp(-1.52554))
exp(0.47721) / (1 - control + (control * exp(0.47721)))
```
Hazard Ratio for race as a factor is 1.452841 or blacks are approximately 1.45 times more likely to commit crime within the 2 years period of study.

```{r}
#Hazard ratio based on sex
exp(0.22127) / (1 - control + (control * exp(0.22127)))
```
```{r}
#Hazard ration based on age
